...
function saveAnswersG3PostOral() {
  saveAnswers();
  window.location.href = 'g3_posttest_summary.html';
}


function saveAnswersG3PreOral() {
  saveAnswers();
  window.location.href = 'g3_pretest_summary.html';
}
...
function saveAndRedirect1() {
  window.location.href = 'g3_pretest_overall.html';


}
function saveAndRedirect2() {
  window.location.href = 'g3_posttest_overall.html';
...
function saveToLocalStorage(response) {
  ...
  if (response.correct_answers && response.correct_answers.literal && response.correct_answers.literal.total_count !== undefined) {
      localStorage.setItem('literalScores', JSON.stringify(response.correct_answers.literal.total_count));
      console.log("literalScores saved:", JSON.stringify(response.correct_answers.literal.total_count));
  } else {
      localStorage.setItem('literalScores', JSON.stringify('N/A'));
      console.log("literalScores saved:", 'N/A');
  }
  ...
  if (response.correct_answers && response.correct_answers.inferential && response.correct_answers.inferential.total_count !== undefined) {
      localStorage.setItem('inferentialScores', JSON.stringify(response.correct_answers.inferential.total_count));
      console.log("inferentialScores saved:", JSON.stringify(response.correct_answers.inferential.total_count));
  } else {
      localStorage.setItem('inferentialScores', JSON.stringify('N/A'));
      console.log("inferentialScores saved:", 'N/A');
  }
  ...
  if (response.correct_answers && response.correct_answers.applied && response.correct_answers.applied.total_count !== undefined) {
      localStorage.setItem('appliedScores', JSON.stringify(response.correct_answers.applied.total_count));
      console.log("appliedScores saved:", JSON.stringify(response.correct_answers.applied.total_count));
  } else {
      localStorage.setItem('appliedScores', JSON.stringify('N/A'));
      console.log("appliedScores saved:", 'N/A');
  }

  // Store predicted reading level
  localStorage.setItem('readingLevel', JSON.stringify(response.predicted_comprehension_level ? response.predicted_comprehension_level : 'N/A'));
  console.log("readingLevel saved:", JSON.stringify(response.predicted_comprehension_level ? response.predicted_comprehension_level : 'N/A'));

  // Store predicted reading level
  localStorage.setItem('postreadingLevel', JSON.stringify(response.predicted_comprehension_level ? response.predicted_comprehension_level : 'N/A'));
  console.log("postreadingLevel saved:", JSON.stringify(response.predicted_comprehension_level ? response.predicted_comprehension_level : 'N/A'));

  // Store literal feedback
  localStorage.setItem('literalFeedback', JSON.stringify(response.feedback && response.feedback.literal ? response.feedback.literal : 'No literal feedback available'));
  console.log("literalFeedback saved:", JSON.stringify(response.feedback && response.feedback.literal ? response.feedback.literal : 'No literal feedback available'));

  // Store inferential feedback
  localStorage.setItem('inferentialFeedback', JSON.stringify(response.feedback && response.feedback.inferential ? response.feedback.inferential : 'No inferential feedback available'));
  console.log("inferentialFeedback saved:", JSON.stringify(response.feedback && response.feedback.inferential ? response.feedback.inferential : 'No inferential feedback available'));

  // Store applied feedback
  localStorage.setItem('appliedFeedback', JSON.stringify(response.feedback && response.feedback.applied ? response.feedback.applied : 'No applied feedback available'));
  console.log("appliedFeedback saved:", JSON.stringify(response.feedback && response.feedback.applied ? response.feedback.applied : 'No applied feedback available'));

  // Store level feedback
  localStorage.setItem('levelFeedback', JSON.stringify(response.feedback && response.feedback.level ? response.feedback.level : 'No level feedback available'));
  console.log("levelFeedback saved:", JSON.stringify(response.feedback && response.feedback.level ? response.feedback.level : 'No level feedback available'));
}